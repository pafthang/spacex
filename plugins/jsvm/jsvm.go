...
func (p *plugin) normalizeServeExceptions() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		jsException, ok := err.Err.(*goja.Exception)
		if !ok {
			return // это не исключение goja
		}

		switch v := jsException.Value().Export().(type) {
		case error:
			c.AbortWithError(http.StatusInternalServerError, v) // прерываем обработку и возвращаем ошибку
		case map[string]any: // goja.GoError
			if vErr, ok := v["value"].(error); ok {
				c.AbortWithError(http.StatusInternalServerError, vErr) // прерываем обработку и возвращаем ошибку
			}
		}
	}
}
