...
func bindRealtimeApi(app core.App, group *gin.RouterGroup) {
	api := realtimeApi{app: app}

	realtime := group.Group("/realtime")
	realtime.GET("", api.connect)
	realtime.POST("", ActivityLogger(app), api.setSubscriptions)
...
func (api *realtimeApi) connect(c *gin.Context) {
...
func (api *realtimeApi) setSubscriptions(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Missing or invalid client id."})
		return
	}

	// check if the previous request was authorized
	oldAuthId := extractAuthIdFromGetter(client)
	newAuthId := extractAuthIdFromContext(c)
	if oldAuthId != "" && oldAuthId != newAuthId {
		c.JSON(http.StatusForbidden, gin.H{"error": "The current and the previous request authorization don't match."})
		return
	}
	...
	api.app.OnRealtimeBeforeSubscribeRequest().Trigger(event, func(e *core.RealtimeSubscribeEvent) error {
		// update auth state
		if admin, exists := e.HttpContext.Get(ContextAdminKey); exists {
			e.Client.Set(ContextAdminKey, admin)
		}
		if authRecord, exists := e.HttpContext.Get(ContextAuthRecordKey); exists {
			e.Client.Set(ContextAuthRecordKey, authRecord)
		}
...
// recordData represents the broadcasted record subscription message data.
...
func extractAuthIdFromContext(c *gin.Context) string {
	record, exists := c.Get(ContextAuthRecordKey)
	if exists {
		if rec, ok := record.(*models.Record); ok && rec != nil {
			return rec.Id
		}
	}
	admin, exists := c.Get(ContextAdminKey)
	if exists {
		if adm, ok := admin.(*models.Admin); ok && adm != nil {
			return adm.Id
		}
	}
	return ""
