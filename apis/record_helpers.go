...
func RequestData(c *gin.Context) *models.RequestInfo {
...
func RequestInfo(c *gin.Context) *models.RequestInfo {
	...
	if v, exists := c.Get(ContextRequestInfoKey); exists {
		if data, ok := v.(*models.RequestInfo); ok {
			// refresh auth state
			value, _ := c.Get(ContextAuthRecordKey)
			data.AuthRecord, _ = value.(*models.Record)
			value, _ = c.Get(ContextAdminKey)
			data.Admin, _ = value.(*models.Admin)
			return data
		}
	}
	...
	value, _ := c.Get(ContextAuthRecordKey)
	result.AuthRecord, _ = value.(*models.Record)
	value, _ = c.Get(ContextAdminKey)
	result.Admin, _ = value.(*models.Admin)
	c.BindQuery(&result.Query)
...
func RecordAuthResponse(
	...
	return app.OnRecordAuthRequest().Trigger(event, func(e *core.RecordAuthEvent) error {
		...
		expands := strings.Split(c.Query(expandQueryParam), ",")
		if len(expands) > 0 {
		...
		e.HttpContext.JSON(http.StatusOK, result)
		return nil
	})
}
...
func EnrichRecord(c *gin.Context, dao *daos.Dao, record *models.Record, defaultExpands ...string) error {
...
//     are visible only for the current logged admin, record owner or record with manage access
func EnrichRecords(c *gin.Context, dao *daos.Dao, records []*models.Record, defaultExpands ...string) error {
	...
	if param := c.Query(expandQueryParam); param != "" {
