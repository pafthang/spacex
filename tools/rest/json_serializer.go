...
// Serializer represents custom REST JSON serializer with support for additional
// generic response data transformation (eg. fields picker).
...
func (s *Serializer) Serialize(c *gin.Context, i any, indent string) error {
	...
	rawFields := c.Query(fieldsParam)
	if rawFields == "" || statusCode < 200 || statusCode > 299 {
		return s.defaultSerialize(c, i, indent)
	}
...
func (s *Serializer) defaultSerialize(c *gin.Context, i any, indent string) error {
